...
//#include "log/log_tool_android.h"
...
static int kControllerSwapInterval = 100;
...
typedef struct MockController
{
    ...
    void Update()
    {
        ...
        if (fabs(position.x) > kControllerMaxDistance) {
            if (isLeft) {
                position.x *= -1.0F;} else { position.x *= 1.0F;}
        }
...
typedef struct InputProviderData
{
    void Reset()
    {
        ...
        lControllerCharacteristics = leftControllerCharacteristics;
        rControllerCharacteristics = rightControllerCharacteristics;
        hmd.Reset();
        lController.Reset();
        rController.Reset();

        lController.isLeft = true;
        rController.isLeft = false;
    }

    unsigned int frameCount;

    UnityXRInputDeviceCharacteristics lControllerCharacteristics;
    UnityXRInputDeviceCharacteristics rControllerCharacteristics;
    MockHMD hmd;
    MockController lController;
    MockController rController;
...
{
    kDeviceId_HMD = 0,
    kDeviceId_Controller_left,
    kDeviceId_Controller_right
};
...
static UnitySubsystemErrorCode Tick(UnitySubsystemHandle handle, void* userData, UnityXRInputUpdateType updateType)
{
    ...
    if (s_ProviderData.frameCount == kControllerConnectedFrameCount) {
        s_XrInput->InputSubsystem_DeviceConnected(handle, kDeviceId_Controller_left);
        s_XrInput->InputSubsystem_DeviceConnected(handle, kDeviceId_Controller_right);
    }
...
//    }
    ...
    s_ProviderData.lController.Update();
    s_ProviderData.rController.Update();
...
static UnitySubsystemErrorCode FillDeviceDefinition(UnitySubsystemHandle handle, void* userData, UnityXRInternalInputDeviceId deviceId, UnityXRInputDeviceDefinition* definition)
{
    ...
    break;
    case kDeviceId_Controller_left:
    {
        s_XrInput->DeviceDefinition_SetName(definition, "Sample Left Controller");

        s_XrInput->DeviceDefinition_SetCharacteristics(definition, s_ProviderData.lControllerCharacteristics);
        ...
        s_XrInput->DeviceDefinition_AddFeatureWithUsage(definition, "Menu Button", kUnityXRInputFeatureTypeBinary, kUnityXRInputFeatureUsageMenuButton);
        ...
        s_XrInput->DeviceDefinition_AddFeatureWithUsage(definition, "Secondary 2D Axis", kUnityXRInputFeatureTypeAxis2D, kUnityXRInputFeatureUsageSecondary2DAxis);

        UnityXRInputFeatureIndex featureIndex1 = s_XrInput->DeviceDefinition_AddFeature(definition, "Unmapped Button", kUnityXRInputFeatureTypeBinary);
        s_XrInput->DeviceDefinition_AddUsageAtIndex(definition, featureIndex1, kUnityXRInputFeatureUsageTriggerButton);
    ...
    break;
    case kDeviceId_Controller_right:
    {
        s_XrInput->DeviceDefinition_SetName(definition, "Sample Right Controller");

        s_XrInput->DeviceDefinition_SetCharacteristics(definition, s_ProviderData.rControllerCharacteristics);
...
static UnitySubsystemErrorCode UpdateDeviceState(UnitySubsystemHandle handle, void* userData, UnityXRInternalInputDeviceId deviceId, UnityXRInputUpdateType updateType, UnityXRInputDeviceState* state)
{
    ...
    case kDeviceId_Controller_left:
    {
        MockController* controller = &s_ProviderData.lController;
    ...
    case kDeviceId_Controller_right:
    {
        MockController* controller = &s_ProviderData.rController;
        s_XrInput->DeviceState_SetBinaryValue(state, featureIndex++, controller->isTracked);
        s_XrInput->DeviceState_SetDiscreteStateValue(state, featureIndex++, controller->trackingState);
        s_XrInput->DeviceState_SetAxis3DValue(state, featureIndex++, controller->position);
...
static UnitySubsystemErrorCode HandleRecenter(UnitySubsystemHandle handle, void* userData)
{
    ...
    s_ProviderData.lController.Recenter();
    s_ProviderData.rController.Recenter();
...
static void UNITY_INTERFACE_API Lifecycle_Stop(UnitySubsystemHandle handle, void* data)
{
    ...
    s_XrInput->InputSubsystem_DeviceDisconnected(handle, kDeviceId_Controller_left);
    s_XrInput->InputSubsystem_DeviceDisconnected(handle, kDeviceId_Controller_right);
}
